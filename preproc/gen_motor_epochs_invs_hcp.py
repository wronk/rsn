"""
gen_motor_epochs_invs_hcp.py

@author: wronk

Script to loop over HCP subjects, compute motor epochs, and compute inverses
"""
...
hcp_params = dict(hcp_path=hcp_path, data_type='task_motor')
...
for subj_fold in dirs:
    ...
    for run_index, events in zip(motor_params['runs'], all_events):
        ...
        epochs = mne.Epochs(raw, events=events,
                            event_id=motor_params['event_id'],
                            tmin=motor_params['tmin'],
                            tmax=motor_params['tmax'],
                            reject=None, baseline=None,
                            decim=motor_params['decim'], preload=True)
        ...
        epo_fname = op.join(epo_fold, '%s_motor_run%i-epo.fif' %
                            (subj_fold, run_index))
    ...
    write_inv(op.join(inv_fold, '%s_motor-inv.fif' % subj_fold), inv)
