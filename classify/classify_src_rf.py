...
Classify whether subject was resting or doing task using random forests from
sklearn
"""
...
hyper_params = [RF_P['n_est_range'], RF_P['max_feat_range'], range(RF_P['n_repeats'])]
...
for si, s_num in enumerate(subj_nums):
    ...
    for hpi, hp_set in zip(list(product(*hyper_param_inds)),
        ...
        # Train and predict
        #########################
        cv_obj = StratifiedKFold(y_labels, n_folds=RF_P['n_folds'],
                                 shuffle=True)
...
if save_data:
    ...
    fname_scores_pkl = op.join(data_head, 'rsn_results', 'rf_scores_' +
